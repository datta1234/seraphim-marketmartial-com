...
trait CalculatesForEfp {
    ...
    {     
        ...
        $totalFee = round(abs(round($future1 * ($is_sender ? $D1efpFeeSender : $D1efpFeeReceiving) * $FutBrodirection1, 5)) * $contracts * 10);
        // Calculate for the counter
        $totalFeeCounter = round(abs(round( ($FuturesSpotRef1 * ($is_sender ? $D1efpFeeReceiving : $D1efpFeeSender) * $counterFutBrodirection1), 5)) * $contracts * 10);

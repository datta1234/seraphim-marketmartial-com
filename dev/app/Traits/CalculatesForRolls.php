...
trait CalculatesForRolls {
    ...
    {     
        ...
        $totalFee = round(abs(round($future2 * ($is_sender ? $D1rollFeeSender : $D1rollFeeReceiving) * $FutBrodirection2, 5)) * $contracts * 10);
        // Calculate for the counter
        $totalFeeCounter = round(abs(round( ($RollNearFutureRef * ($is_sender ? $D1rollFeeReceiving : $D1rollFeeSender) * $counterFutBrodirection2), 5)) * $contracts * 10);

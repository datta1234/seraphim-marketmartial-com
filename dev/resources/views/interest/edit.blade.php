									...
									<div class="form-group row">
										<div class="col-md-12">
											{{ Form::label($interest->id,$interest->title, ['class' => 'col-form-label']) }}
												<activate-input :active="{{ ($interest->pivot != null) || is_array(old('interest')) && array_key_exists($index,old('interest'))? 'true': 'false' }}" placeholder="team (int. and/or club)" name="{{ 'interest['.$interest->id.'][value]' }}" value="{{ $interest->pivot ? $interest->pivot->value : null }}">
												</activate-input>

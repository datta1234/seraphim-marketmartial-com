<template>
    <div dusk="market-tab-rolls"  class="col market-tab market-tab-rolls p-3 mb-2 mt-2" v-bind:class="marketState" @click="loadInteractionBar()">
                ...
                <b-row no-gutters align-v="center" align-h="center">
                    <b-col cols="auto" class="text-center">
                        <div class="market-tab-strikes">
                            <div v-html="tradable_1" class="text-center"></div>
                            <span v-html="expiry_1"></span>&nbsp;-&nbsp;<span v-html="expiry_2"></span>
                        </div>
                    </b-col>
                </b-row>
            </b-col>
...
<script>
    ...
    export default {
        ...
        computed: {
            tradable_1: function() {
                let group = this.$root.config("trade_structure.rolls.group_1");
                return this.marketRequest.trade_items[group].tradable.title;
            },
            expiry_1: function() {
                let group = this.$root.config("trade_structure.rolls.group_1");
                let exp = this.$root.config("trade_structure.rolls.expiration_date_1");
                return this.marketRequest.trade_items[group][exp];
            },
            expiry_2: function() {
                let group = this.$root.config("trade_structure.rolls.group_1");
                let exp = this.$root.config("trade_structure.rolls.expiration_date_2");
                return this.marketRequest.trade_items[group][exp];
            },

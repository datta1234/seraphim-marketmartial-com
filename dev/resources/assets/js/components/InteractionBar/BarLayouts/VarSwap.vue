    ...
    export default {
        ...
        computed: {
            'market_title': function() {
                ...
                let expiration_date = this.$root.config("trade_structure.var_swap.expiration_date");
                let cap = this.$root.config("trade_structure.var_swap.cap");
                return [
                    this.marketRequest.trade_items[group].tradable.title,
                    this.marketRequest.trade_items[group][expiration_date],
                    this.marketRequest.trade_structure,
                    '('+( this.marketRequest.trade_items[group][cap] == 0 ? 'Uncapped' : ''+this.marketRequest.trade_items[group][cap]+'x Cap')+')'
                ].join(' ');
            },
        },
        methods: {
